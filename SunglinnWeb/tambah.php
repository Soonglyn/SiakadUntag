...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Proses upload file
    if (isset($_FILES['foto_ijazah']) && $_FILES['foto_ijazah']['error'] == 0) {
        $upload_dir = 'uploads/';
        $foto = basename($_FILES['foto_ijazah']['name']);
        ...
        if (move_uploaded_file($_FILES['foto_ijazah']['tmp_name'], $target_file)) {
            $foto_ijazah = $foto;
        } else {
            echo "<script>alert('Gagal mengunggah foto ijazah.');</script>";
        }
    }

    // Masukkan data ke database menggunakan prepared statement
    $stmt = $conn->prepare("INSERT INTO mahasiswa (nbi, nama, alamat, ipk, spp, prodi, foto_ijazah) 
                            VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssss", $nbi, $nama, $alamat, $ipk, $spp, $prodi, $foto_ijazah);
...
<div class="content" id="content">
    ...
    <form method="POST" action="" enctype="multipart/form-data">
        ...
        <input type="text" id="nbi" name="nbi" required>
        ...
        <input type="text" id="spp" name="spp" required>

        <label for="prodi">Prodi:</label>
        <input type="text" id="prodi" name="prodi" required>

        <label for="foto_ijazah">Foto Ijazah:</label>
        <input type="file" id="foto_ijazah" name="foto_ijazah" required>

        <a>
        <button type="submit" class="btn btn-success">Tambah</button>
        </a>
        <a href="mahasiswa.php" class="btn btn-secondary mb-3">Batal</a>
    </form>
</div>
</body>
</html>
